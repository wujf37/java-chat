...
public class GetSideRecordTask implements Runnable{
	String starttime;
	String userId1;
	String userId2;
	Socket server;
	...
	public GetSideRecordTask(String[] arg,Socket userver,Connection ucon,Hashtable<String,User> uht) {
	...
	public void run() {
		System.out.println("用户"+userId1+"获取与用户"+userId2+"的聊天历史");
        try
        {   
        	String result="";
            PreparedStatement pstmt=con.prepareStatement("SELECT * FROM siderecord WHERE ((senderid = ? AND recieverid = ?)OR(senderid = ? AND recieverid = ?)) AND time < ? ORDER BY time desc Limit 50");
